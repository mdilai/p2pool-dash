...
class WorkerBridge(worker_interface.WorkerBridge):
    ...
    def __init__(self, node, my_pubkey_hash, donation_percentage, merged_urls,
                 worker_fee, min_difficulty, share_rate, share_rate_type, args, pubkeys, dashd):
    ...
    def get_work(self, user, pubkey_hash, desired_share_target, desired_pseudoshare_target):
        ...
        if desired_pseudoshare_target is None:
            ...
            if self.share_rate is not None:
                ...
                if local_rate > 0:
                    target = min(target, dash_data.average_attempts_to_target(local_rate * 60 / self.share_rate))
            else: # per-node
        ...
        def got_response(header, user, coinbase_nonce):
            ...
            if pow_hash <= share_info['bits'].target and header_hash not in received_header_hashes:
                ...
                # node.py will sometimes forget transactions if dashd's work has changed since this stratum
